...
@Component({ standalone: true, imports: [FormsModule, RouterLink], template: `<h1>Prijatelji</h1><form (ngSubmit)="follow()" aria-label="Dodavanje prijatelja"><label for="friend-username">Username prijatelja</label><input id="friend-username" name="username" [(ngModel)]="username" required><button type="submit">Pošalji zaprati</button></form>@if (message) {<p role="status" aria-live="polite">{{ message }}</p>}<h2>Aktivnosti prijatelja</h2><ul aria-label="Aktivnosti prijatelja">@for (activity of activities; track activity.username + activity.bookTitle + activity.updatedAt) {<li><strong>{{ activity.username }}</strong> {{ activity.status === 'read' ? 'označio/la je kao pročitanu' : 'želi da pročita' }} knjigu <em>{{ activity.bookTitle }}</em></li>} @empty {<li>Nema aktivnosti prijatelja.</li>}</ul><h2>Lista prijatelja</h2><ul aria-label="Lista prijatelja">@for (friend of friends; track friend.id) {<li><a [routerLink]="['/profile/user', friend.id]">{{ friend.username }}</a></li>} @empty {<li>Još nemaš prijatelje.</li>}</ul>` })
